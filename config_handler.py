...
class ConfigHandler:
    ...
    def get_theme_mode(self) -> str:
        ...
        Returns:
            Theme mode string ("dark" or "light")
        """
        mode = self.config.get("theme_mode", "dark")
        # Normalize old "modern" to "dark"
        if mode == "modern":
            mode = "dark"
    ...
    def set_theme_mode(self, mode: str) -> None:
        ...
        Args:
            mode: Theme mode identifier ("dark" or "light")
        """
        if mode.lower() in ["dark", "light"]:
            self.config["theme_mode"] = mode.lower()
        else:
            self.config["theme_mode"] = "dark"  # Default to dark
        self.save_config()
